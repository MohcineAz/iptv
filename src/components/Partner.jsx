...
function Partner() {
  return (
    <section className="bg-gradient-to-r from-[#00050E] from-30% via-[#8000FF] via-100% to-[#AD00FF] to-80% py-12 px-8 grid">
        <div className="grid grid-cols-2 md:grid-cols-6 sm:grid-cols-3  gap-4 overflow-hidden space-y-5">
            {
                channels.map((channel) =>
                    <img key={channel.id} src={channel.src} alt="" />
                )
            }
        </div>
    </section>
